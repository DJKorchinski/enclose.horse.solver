...
- Keep inputs in the repo root: `example_map.txt`, `portal_map.txt`, and `example.png` remain the canonical fixtures; do not overwrite them.  
...
- Example local run (once CLI exists): `python -m enclose_horse.cli --map example_map.txt --max-walls 13 --plot out.png`.
...
## Testing Guidelines
- Use `pytest` with test files named `test_*.py`; prefer small fixtures derived from `example_map.txt`/`portal_map.txt`.  
- Validate the parser by round-tripping `example_map.txt` against `example.png` and by asserting the known optimal score (103 with 13 walls) once the ILP solver is implemented.  
